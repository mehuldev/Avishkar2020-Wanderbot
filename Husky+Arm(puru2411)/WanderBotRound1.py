...
def follow_trajectory(id):
	...
	while t <= 2*np.pi+.1:
		...
		q1, q2, q3, q4, q5, q6 = get_kuka_angles(kukaBasePosition, kukaBaseOrientation, [px, py, pz], [0, 0, 0])
...
def get_path(id):
	...
	pts = [ ((p.real-100)/200,(300-p.imag)/200) for p in (path.point(i/n) for i in range(0, n+1))]  
...
def write():		# will write "ROBOTICS"
	...
	arr = [[-6,0], [-6,2], [-5,2], [-5,1], [-6,1], [-5,0], [-3,0], [-3,2], [-4,2], [-4,0], [-1,0], [-1,2], [-2,2], [-2,1], [-1,1], [-2,1], [-2,0], [1,0], [1,2], [0,2], [0,0], [3,0], [3,2], [2,2], [4,2], [3,2], [3,0], [5,0], [5,2], [5,0], [6,0], [6,2], [7,2], [6,2], [6,0], [8,0], [9,0], [9,1], [8,1], [8,2], [9.5,2]]
...
for i in range(len(positionsCube)):
	print("going to pick the cube from : ", positionsCube[i][0], ", ", positionsCube[i][1])
	pick_cube_from([positionsCube[i][0], positionsCube[i][1], 0])
	print("going to place the cube from : ", positionsPatch[i][0], ", ", positionsPatch[i][1])
